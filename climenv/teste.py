...
data_i = datetime(2026,4,1,00,00,00)
...
def plot_com_datas_reais(df, municipio, nome_sistema):
    ...
    ax2.set_ylabel('Temperatura Máx. Diária (°C)', fontsize=10)
...
for i,j in municipios:
	...
	municipio_samet = samet(f"{path_dados}/samet_tmp2m_hor_{i}_{data_i.strftime('%Y%m%d%H')}.txt")
	municipio_epagri = epagri(f"{path_dados}/{i}_epagri.csv")
	municipio_merge = merge(f"{path_dados}/merge_prec_hor_{i}_{data_i.strftime('%Y%m%d%H')}.txt") 
