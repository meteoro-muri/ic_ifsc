...
data_i = datetime(2026,4,1,00,00,00) #ALTERAR APENAS AQUI A DATA 
data_a = data_i - timedelta(days=7)
data_f = data_i + relativedelta(months=+1) #- timedelta(days=1)
...
def plot_com_datas_reais(df, municipio, nome_sistema):
    ...
    ax2.grid(True, alpha=0, axis='both')
    ...
    plt.savefig(f"{path_save}/mensal_{nome_sistema}_{data_i.strftime('%Y%m')}.png")
    plt.show()
...
for i,j in municipios:
	...
	municipio_samet = samet(f"{path_dados}/samet_merge/samet_tmp2m_hor_{i}_{data_i.strftime('%Y%m%d%H')}_31.txt")
	#print(municipio_samet)
	#sys.exit()
	municipio_epagri = epagri(f"{path_dados}/{i}_epagri.csv")
	municipio_merge = merge(f"{path_dados}/samet_merge/merge_prec_hor_{i}_{data_i.strftime('%Y%m%d%H')}_31.txt") 
