...
semana_merge = pd.DataFrame()
semana_merge = pd.concat([semana09, semana16, semana23, semana02], axis=1)
print(semana_merge)

semana_merge["S1"] = semana_merge['2025-02-16']-semana_merge['2025-02-09']
semana_merge["S2"] = semana_merge['2025-02-23']-semana_merge['2025-02-16']
semana_merge["S3"] = semana_merge['2025-03-02']-semana_merge['2025-02-23']
...
semana_merge_melt_poli = pd.merge(semana_merge["S3"], xy, on = "Município", how = "left")
semana_merge_melt_poligeo = gpd.GeoDataFrame(semana_merge_melt_poli, geometry = "geometry", crs = "EPSG:4674")
color = semana_merge_melt_poligeo["S3"]
color = color.to_list()
print(color)
color = pd.DataFrame(color, columns = ["cor"])
semana_merge_melt_poligeo["color"] = color

semana_merge_melt_poligeo["color"]
#sys.exit()
...
vmin = semana_merge_melt_poligeo["color"].min()
vmax = semana_merge_melt_poligeo["color"].max()

absmax = max(abs(vmin), abs(vmax))
norm = TwoSlopeNorm(vmin=-absmax, vcenter=0, vmax=absmax)

base =  semana_merge_melt_poligeo.plot(column = "color", legend=True, cmap = cmap, norm = norm, legend_kwds={'shrink': 0.6})
